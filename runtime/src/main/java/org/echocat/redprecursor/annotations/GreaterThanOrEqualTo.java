...
import org.echocat.redprecursor.annotations.GreaterThanOrEqualTo.Evaluator;
...
public @interface GreaterThanOrEqualTo {
    ...
    /**
     * The property to must greater than or equal to this value.
     */
    public double value();
    ...
    public static class Evaluator implements AnnotationEvaluator<GreaterThanOrEqualTo, Number> {

        @Override
        public void evaluate(@Nonnull GreaterThanOrEqualTo annotation, @Nonnull ElementType elementType, @Nonnull String elementName, @Nullable Number value) {
            if (value == null || !(value.doubleValue() >= annotation.value())) {
                throwMessageFor(annotation, " is not greater than or equal to " + annotation.value() + ".", elementType, elementName, value);
            }
        }
    }
}
